...
// .wrangler/tmp/bundle-YhQ1Vw/checked-fetch.js
...
// .wrangler/tmp/bundle-YhQ1Vw/strip-cf-connecting-ip-header.js
...
function resolveScaleFactor(inputs) {
  ...
  if (bc === tc)
    return toCanonical(ta, tu) / toCanonical(ba, bu);
  if (!cf)
    throw new Error(`Custom conversion required for ${bu} to ${tu}`);
...
__name(resolveScaleFactor, "resolveScaleFactor");
var UnitSchema = external_exports.enum(["g", "kg", "oz", "lb", "ml", "L", "tsp", "tbsp", "cup", "serving", "piece", "item"]);
var RequestSchema = external_exports.object({
  food_name: external_exports.string().optional().default(""),
  protein_g: external_exports.number().min(0),
  calories: external_exports.number().min(0.01),
  base_serving_size: external_exports.number().min(0.01),
  base_serving_unit: UnitSchema,
  target_serving_size: external_exports.number().min(0.01),
  target_serving_unit: UnitSchema,
  custom_conversion: external_exports.object({ from_unit: UnitSchema, to_unit: UnitSchema, value: external_exports.number().min(1e-3) }).nullable().optional()
});
var app = new Hono2();
app.use("*", cors());
app.post("/calculate-protein-ratio", async (c) => {
  try {
    ...
    const scale = resolveScaleFactor(inputs);
    ...
    return c.json({
      ...
      rating,
      scale_position: density >= 10 ? 0.95 : 0.5,
...
// .wrangler/tmp/bundle-YhQ1Vw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-YhQ1Vw/middleware-loader.entry.ts
